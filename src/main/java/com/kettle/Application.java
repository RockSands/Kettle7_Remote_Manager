...
import com.kettle.meta.build.SyncTablesDatasBuilder;
	...
	public static void main(String[] args) throws Exception {
		...
		while (kettleResults.isEmpty()) {
			...
			for (Iterator<KettleResult> it = kettleResults.iterator(); it.hasNext();) {
				...
				System.out.println("任务[" + roll.getUuid() + "]任务状态:" + roll.getStatus());
				if (KettleVariables.RECORD_STATUS_ERROR.equals(roll.getStatus())
						|| KettleVariables.RECORD_STATUS_FINISHED.equals(roll.getStatus())) {
					service.deleteJob(result.getUuid());
					it.remove();
				}
			}
